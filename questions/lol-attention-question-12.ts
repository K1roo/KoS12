...
@Injectable()
export class LolAttentionQuestion12 extends LolAttentionQuestion {
  private _logger: WinstonLogger = new WinstonLogger(
    LolAttentionQuestion12.name,
  );
  public readonly id = 12;
  public readonly title = 'How many Kills did {{streamerName}} have last game?';
  ...
  public readonly build = async (
    ...
    const streamerKills = stats.filter(
      (statEvent) =>
    ...
    switch (difficulty) {
      case QuestionDifficulty.EASY:
        if (streamerKills < 10) correctAnswerIndex = 0;
        if (streamerKills >= 10) correctAnswerIndex = 1;
        break;
      case QuestionDifficulty.EASY:
        if (streamerKills >= 20) correctAnswerIndex = 3;
        if (streamerKills >= 15) correctAnswerIndex = 2;
        if (streamerKills >= 10) correctAnswerIndex = 1;
        if (streamerKills < 10) correctAnswerIndex = 0;
        break;
