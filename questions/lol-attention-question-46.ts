...
@Injectable()
export class LolAttentionQuestion46 extends LolAttentionQuestion {
  private _logger: WinstonLogger = new WinstonLogger(
    LolAttentionQuestion46.name,
  );
  public readonly id = 46;
  public readonly title =
    'Which team secured the first rift herald during the game?';
  ...
  public readonly build = async (
    ...
    if (
      ...
      (typeof firstHeraldKillEvent.data.stats.val !== 'object' ||
        typeof firstHeraldKillEvent.data.stats.val.KillerName !== 'string')
    ) {
    ...
    if (firstHeraldKillEvent) {
      ...
      const killerName = firstHeraldKillEvent.data.stats.val.KillerName;
