...
@Injectable()
export class LolAttentionQuestion23 extends LolAttentionQuestion {
  private _logger: WinstonLogger = new WinstonLogger(
    LolAttentionQuestion23.name,
  );
  public readonly id = 23;
  public readonly title = 'How many barons did {{streamerName}} last hit?';
  ...
  public readonly build = async (
    ...
    const baronsKilledAmount = stats.filter(
      (statEvent) =>
        typeof statEvent.data.stats.val === 'object' &&
    ...
    if (baronsKilledAmount >= step) correctAnswerIndex = getRandomInt(0, 1);
    if (baronsKilledAmount >= step * 2) correctAnswerIndex = getRandomInt(0, 2);
    if (baronsKilledAmount >= step * 3) correctAnswerIndex = getRandomInt(0, 3);
    const options = new Array(4).fill('');
    let j = 0;
    for (let i = correctAnswerIndex; i < options.length; i++) {
      switch (difficulty) {
        case QuestionDifficulty.MEDIUM:
          options[i] = `${j * step + baronsKilledAmount}+`;
          break;
        case QuestionDifficulty.DIFFICULT:
          options[i] = `${j * step + baronsKilledAmount}+`;
          break;
    ...
    if (correctAnswerIndex !== 0) {
      ...
      for (let i = correctAnswerIndex - 1; i >= 0; i--) {
        switch (difficulty) {
          case QuestionDifficulty.MEDIUM:
            options[i] = `${baronsKilledAmount - k * step}+`;
            break;
          case QuestionDifficulty.DIFFICULT:
            options[i] = `${baronsKilledAmount - k * step}+`;
            break;
