...
@Injectable()
export class LolAttentionQuestion13 extends LolAttentionQuestion {
  private _logger: WinstonLogger = new WinstonLogger(
    LolAttentionQuestion13.name,
  );
  public readonly id = 13;
  public readonly title =
    'How many Deaths did {{streamerName}} have last game?';
  ...
  public readonly build = async (
    ...
    const streamerDeaths = stats.filter(
      (statEvent) =>
    ...
    switch (difficulty) {
      case QuestionDifficulty.EASY:
        if (streamerDeaths < 10) correctAnswerIndex = 0;
        if (streamerDeaths >= 10) correctAnswerIndex = 1;
        break;
      case QuestionDifficulty.EASY:
        if (streamerDeaths >= 20) correctAnswerIndex = 3;
        if (streamerDeaths >= 15) correctAnswerIndex = 2;
        if (streamerDeaths >= 10) correctAnswerIndex = 1;
        if (streamerDeaths < 10) correctAnswerIndex = 0;
        break;
