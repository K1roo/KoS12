...
@Injectable()
export class LolAttentionQuestion19 extends LolAttentionQuestion {
  private _logger: WinstonLogger = new WinstonLogger(
    LolAttentionQuestion19.name,
  );
  public readonly id = 19;
  public readonly title = 'When was first Baron killed?';
  ...
  public readonly build = async (
    ...
    if (
      !baronKillEvent ||
      typeof baronKillEvent.data.stats.val !== 'object' ||
      !baronKillEvent.data.stats.val.EventTime
    ) {
    ...
    const eventTime = baronKillEvent.data.stats.val.EventTime;
