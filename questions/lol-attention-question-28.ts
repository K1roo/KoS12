...
@Injectable()
export class LolAttentionQuestion28 extends LolAttentionQuestion {
  private _logger: WinstonLogger = new WinstonLogger(
    LolAttentionQuestion28.name,
  );
  public readonly id = 28;
  public readonly title = 'How many turrets did {{streamerName}} last hit?';
  ...
  public readonly correlationTags: LolCorrelationTag[] = ['TurretKilled'];
  ...
  public readonly isRelevant = (stats: NotaryStatsLolMessage[]): boolean => {
    return stats.some(
      (statEvent) =>
        ...
        statEvent.data.stats.val.EventName === 'TurretKilled' &&
  ...
  public readonly build = async (
    ...
    const turretsKilledAmount = stats.filter(
      (statEvent) =>
        typeof statEvent.data.stats.val === 'object' &&
    ...
    if (turretsKilledAmount >= step) correctAnswerIndex = getRandomInt(0, 1);
    if (turretsKilledAmount >= step * 2)
      correctAnswerIndex = getRandomInt(0, 2);
    if (turretsKilledAmount >= step * 3)
    ...
    for (let i = correctAnswerIndex; i < options.length; i++) {
      switch (difficulty) {
        case QuestionDifficulty.MEDIUM:
          options[i] = `${j * step + turretsKilledAmount}+`;
          break;
        case QuestionDifficulty.DIFFICULT:
          options[i] = `${j * step + turretsKilledAmount}+`;
          break;
    ...
    if (correctAnswerIndex !== 0) {
      ...
      for (let i = correctAnswerIndex - 1; i >= 0; i--) {
        switch (difficulty) {
          case QuestionDifficulty.MEDIUM:
            options[i] = `${turretsKilledAmount - k * step}+`;
            break;
          case QuestionDifficulty.DIFFICULT:
            options[i] = `${turretsKilledAmount - k * step}+`;
            break;
