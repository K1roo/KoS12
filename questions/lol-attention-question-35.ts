...
@Injectable()
export class LolAttentionQuestion35 extends LolAttentionQuestion {
  private _logger: WinstonLogger = new WinstonLogger(
    LolAttentionQuestion35.name,
  );
  public readonly id = 35;
  public readonly title = `Did {{streamerName}}'s team get one or more Dragons last game?`;
  ...
  public readonly build = async (
    ...
    if (dragonKillEvents.length) {
      ...
      for (const dragonKillEvent of dragonKillEvents) {
        const killerName = dragonKillEvent.data.stats.val.KillerName;
