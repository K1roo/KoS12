...
@Injectable()
export class LolAttentionQuestion54 extends LolAttentionQuestion {
  private _logger: WinstonLogger = new WinstonLogger(
    LolAttentionQuestion54.name,
  );
  public readonly id = 54;
  public readonly title =
    'When did {{streamerName}} get his second ability point on his Ulti?';
  ...
  public readonly isRelevant = (stats: NotaryStatsLolMessage[]): boolean => {
    return stats.some(
      (statEvent) =>
        ...
        statEvent.data.stats.path[2] === 'R' &&
        statEvent.data.stats.path[3] === 'abilityLevel' &&
        statEvent.data.stats.val === 2,
    );
  };
  public readonly build = async (
    ...
    const [abilityUpdateEvent] = stats
      .filter(
        (statEvent) =>
          statEvent.data.stats.op === 'update' &&
          statEvent.data.stats.path &&
    ...
    if (!streamerName || !abilityUpdateEvent || !gameStartEvent) {
    ...
    const {receivedAt: levelUpdateEventAt} = abilityUpdateEvent.data.stats;
