...
const boostId = 'Lightning' as KosBoostId;

@Boost(boostId)
@Injectable()
export class LightningBoostService extends KosUserBoostService {
  private readonly _logger = new WinstonLogger(LightningBoostService.name);

  public lootWeight = 1;
  public rarity = 1;

  public readonly id = boostId;
  public readonly title = 'Lightning';
  public readonly description =
    'Bonus Score for fast answers! Double Score Bonus if >50% time remains.';
  public readonly iconUrl =
    'https://assets.azarus.io/kos/boosts-icons/lightning-29042021.svg';
  ...
  public getBoostedScoreValue(
    oldVal: number,
    answeredAtMs: number,
    questionStartAtMs: number,
    questionFinishAtMs: number,
  ): number {
    const questionDuration = questionFinishAtMs - questionStartAtMs;
    const remainTime = questionFinishAtMs - answeredAtMs;
    const remainTimeRatio = remainTime / questionDuration;
    if (remainTimeRatio > 0.5) {
  ...
  public async apply(
    ...
    this._logger.debug('Applying Lightning');
    ...
    if (userAnswer && userAnswer.won) {
      ...
      const newValue = this.getBoostedScoreValue(
        userAnswer.starsDistributed,
        userAnswer.emittedAt.getTime(),
