...
const boostId = 'ReverseScore' as KosBoostId;

@Boost(boostId)
@Injectable()
export class ReverseScoreBoostService extends KosUserBoostService {
  private readonly _logger = new WinstonLogger(ReverseScoreBoostService.name);

  public lootWeight = 1;
  public rarity = 0;

  public readonly id = boostId;
  public readonly title = 'ReverseScore';
  public readonly description = 'Swaps the minimum and maximum score values';
  public readonly iconUrl =
    'https://assets.azarus.io/kos/boosts-icons/reverse-score.svg';
  ...
  public async apply(
    ...
    this._logger.debug('Applying ReverseScore');
    ...
    if (userAnswer && userAnswer.won) {
      ...
      const newValue = mapToIntegerInLimits(
        userAnswer.emittedAt.getTime(),
        [waveQuestion.startAt.getTime(), partialQuestionStateToSave.maxScore],
        [waveQuestion.finishAt.getTime(), partialQuestionStateToSave.minScore],
      );
      promises.push(
