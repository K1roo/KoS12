...
const boostId = 'FiftyFifty' as KosBoostId;

@Boost(boostId)
@Injectable()
export class FiftyFiftyBoostService extends KosUserBoostService {
  private readonly _logger = new WinstonLogger(FiftyFiftyBoostService.name);
  ...
  public readonly title = '50/50';
  public readonly description = 'Reduce Choices down to 2';
  public readonly iconUrl =
    'https://assets.azarus.io/kos/boosts-icons/50-50-29042021.svg';
  ...
  public async apply(
    ...
    const now = Date.now();
    this._logger.debug('Applying FiftyFifty');
    ...
    const optionsAmountInButton = Math.ceil(availableOptions.length / 2);
    const newButtonMapper = availableOptions.reduce(
      (acc: number[][], val) => {
        if (acc[acc.length - 1].length < optionsAmountInButton) {
    ...
    this._logger.verbose('New button mapper defined', {
      optionsAmountInButton,
      newButtonMapper,
      availableOptions,
    });
